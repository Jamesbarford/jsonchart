...
static double chartLinearScale(chartScale *cs, double value) {
    return _chartLinearScale(value, cs->valMin, cs->valMax, cs->rangeMin,
            cs->rangeMax);
}
...
int chartLineCalculateYScale(chartDimensions *cDims,
        chartPointArray *cpArr, chartScale *cs)
{
    double val;
    double min, max;
    int i;
    ...
    cs->valMax = max;
    cs->rangeMin = 0;
    cs->rangeMax = cDims->height - cDims->marginTop;
...
static inline void _sortDoubleArray(double *arr, int len) {
    qsort(arr, len, sizeof(double), doublecmp);
}

/**
 * - Sort the array to find smallest and largest: only need to sort the y axis,
 *   as the array is already sorted by time
 * - Find the lowest whole number
 * - Find the largest whole number
 * - fin
 */
double *_chartSortAxisValues(chartPointArray *cpArr) {
    double *cpy;

    if ((cpy = malloc(sizeof(double) * cpArr->len)) == NULL)
        return NULL;

    memcpy(cpy, cpArr->yValues, cpArr->len * sizeof(double));
    _sortDoubleArray(cpy, cpArr->len);
 ...
 * on an SVG chart.
 */
char *_chartLinePointsToString(chartPointArray *cpArr, chartDimensions *cDim,
        chartScale *csy, int *len)
{
    int i, offset;
    char *outstr;
    ...
    offset += sprintf(outstr, "<path fill=\"none\" "
...
char *chartLinePointsToString(chartPointArray *cpArr, chartDimensions *cDims,
        int *len)
{
    chartScale csy;

    // TODO: delete this function it is not needed in its current rendition
    // as we already have min + max from sorting the array
    chartLineCalculateYScale(cDims, cpArr, &csy);

    return _chartLinePointsToString(cpArr, cDims, &csy, len);
}
        ...
        chartFormatter *formatter, double *xTicks, int *outlen)
{
    ...
    *outlen += snprintf(xAxis + *outlen, BUFSIZ,
    ...
    acc = 0;
    tickSpace =
        (double)(cDims->width + cDims->marginRight + cDims->marginRight) / numTicks;

    *outlen += snprintf(xAxis + *outlen, BUFSIZ,
            "<g transform=\"translate(0, %d)\" fill=\"none\" font-size=\"10\""
            " font-family=\"sans-serif\" text-anchor=\"middle\">", cDims->height);
    ...
    for (i = 0; i < numTicks; ++i) {
        ...
        *outlen += snprintf(xAxis + *outlen, BUFSIZ,
            ...
            "<text style=\"font-size: 8px; text-anchor: end;\" fill=\"currentColor\""
        ...
        chartAxisFormatters *formatters, int width, int height, int *outlen)
{
    ...
    int lineBufLen, yAxisLen, xAxisLen, i;
    ...
    *outlen = 0;
    ...
    for (i = 0; i < cpArr.len; ++i) {
        double x = cpArr.xValues[i];
        double y = cpArr.yValues[i];
        printf("x:%.2f, y:%.2f\n", x,y);
    ...
    if ((lineBuf = chartLinePointsToString(&cpArr, &cDims, &lineBufLen))
            == NULL)
        goto svg_finalise;

    if ((svgbuf = malloc(sizeof(char *) * (BUFSIZ + yAxisLen + xAxisLen + lineBufLen)))
            == NULL)
        goto svg_finalise;
...
static char *_chartMultiCreateSVG(int arrayCount, chartPointArray *cpArrays,
        chartDimensions *cDims, chartFormatter *yFormatter, chartFormatter *xFormatter,
        double *xTicks, int xTickCount, double *yTicks, int yTickCount,
        chartScale *csy, int *outlen)
{
    ...
    for (i = 0; i < arrayCount; ++i) {
        ...
        if ((lineChartBuffers[i] = chartLinePointsToString(cpArr, cDims,
                        &lineBufLen)) == NULL)
            goto chart_finalise;
    }

    /* Step 2: create axis buffers */
    if ((xAxis = chartXAxisCreate(cDims, xTickCount, xFormatter, xTicks,
                    &xAxisLen)) == NULL)
        goto chart_finalise;

    if ((yAxis = chartYAxisCreate(cDims, yTickCount, yFormatter, yTicks,
                    &yAxisLen)) == NULL)
        goto chart_finalise;

    /* Step 3: create SVG buffer */
    svgbufSize = sizeof(char) * ((lineBufLen * arrayCount) + xAxisLen + yAxisLen);
    ...
    *outlen = snprintf(svgbuf, svgbufSize,
            ...
            cDims->width, cDims->height,
            xAxis, yAxis);

    for (j = 0; j < arrayCount; ++j) {
        *outlen += snprintf(svgbuf + *outlen, svgbufSize, "%s",
                lineChartBuffers[j]);
    }
...
static char *_chartMultiCalculateAxisAndCreateSVG(chartDimensions *cDims,
        int arrayCount, chartPointArray *cpArrays,
        chartFormatter *yFormatter, chartFormatter *xFormatter, int *outlen)
{
    chartPointArray *cpArr;
    chartScale csy;
    int i, j;
    ...
    svgbuf = _chartMultiCreateSVG(arrayCount, cpArrays, cDims, yFormatter, xFormatter,
            xTicks, 5, yTicks, 12, &csy, outlen);
        ...
        chartAxisFormatters *formatters, int *outlen)
{
    ...
    svgbuf = _chartMultiCalculateAxisAndCreateSVG(&cDims, arrayCount, cp_arrays,
            yFormatter, xFormatter, outlen);
...
static int getValueType(char *strtype) {
    if (strncmp(strtype, "string", 5 ) == 0) return J_STRING;
    if (strncmp(strtype, "long", 4) == 0 ||
        strncmp(strtype, "int", 3) == 0) return J_LONG;
    if (strncmp(strtype, "float", 5) == 0 ||
            strncmp(strtype, "double", 6) == 0) return J_FLOAT;
    return -1;
}

static char *getValueName(int j_type) {
    switch (j_type) {
        case J_FLOAT: return "float";
        case J_LONG: return "long";
...
static int printAxisTypeWarning(char axis) {
    fprintf(stderr, "ERROR: --%c-type must be one of "
...
int main(int argc, char **argv) {
    ...
    char *x_value_name, *y_value_name, *filename, *out_filename, *raw_json, *svgbuf;
    char chartname[200];
    ...
    if ((svgbuf = chartLineCreateSVG(xValues, yValues, arr_size, NULL,
                    width, height, &svgbuf_len)) == NULL) {
